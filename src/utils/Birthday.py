...
class Birthday(commands.Cog, name="Birthdays!"):
    ...
    def __init__(self, client):
        ...
        curr.execute(
            """CREATE TABLE IF NOT EXISTS Birthdays ([MemberID] INTEGER, [GuildID] INTEGER, [Birthday] TIMESTAMP, PRIMARY KEY ("MemberID","GuildID"))"""
        )
        db.close()
    ...
    async def on_member_remove(self, member):
        async with aiosqlite.connect(self.RT_DATABASE) as db:
            async with db.cursor() as curr:
                await curr.execute(
                    """DELETE FROM Birthdays WHERE GuildID=:guildID AND MemberID=:memberID""",
    ...
    async def on_guild_remove(self, guild: discord.Guild):
        async with aiosqlite.connect(self.RT_DATABASE) as db:
            async with db.cursor() as curr:
                await curr.execute(
                    """DELETE FROM Birthdays WHERE GuildID=:guildID""",
                    {"guildID": guild.id},
                )
                await curr.execute(
                    """DELETE FROM BirthdayMessage WHERE GuildID=:guildID""",
                    {"guildID": guild.id},
                )
                await db.commit()
        ...
        async with aiosqlite.connect(self.RT_DATABASE) as db:
            async with db.cursor() as curr:
                if ctx.message.channel.type is discord.ChannelType.text:
                    ...
                    await curr.execute(
                        """INSERT INTO BirthdayMessage (GuildID, ChannelID)
                                VALUES (:guildid, :channelid)""",
                        {"guildid": ctx.guild.id, "channelid": ctx.message.channel.id},
                    )
                    await db.commit()
                    await ctx.send(
                        "Success. This channel will now be used for birthday messages!"
                    )
                else:
    ...
    async def clearBirthday(self, ctx):
        async with aiosqlite.connect(self.RT_DATABASE) as db:
            async with db.cursor() as curr:
                ...
                await db.commit()
                await ctx.send(
                    "Success. Removed the birthday messages from this channel!"
                )

    @commands.command(
        brief="Set your birthday",
        description='Set your birthday!. The bot will send a message when its time \n\
        ...
        async with aiosqlite.connect(self.RT_DATABASE) as db:
            async with db.cursor() as curr:
                server = await db.execute(
                    """SELECT GuildID, ChannelID FROM BirthdayMessage WHERE GuildID=:guildid""",
                    {"guildid": ctx.guild.id},
                )
                server_info = await server.fetchone()
                if server_info == None:
                    await ctx.send(
                        "This server has not been set up for birthday messages, yet!"
                    )
                    return
                ...
                if args[0].isnumeric() and args[1].isnumeric() and args[2].isnumeric():
                    try:
                        await curr.execute(
                            """DELETE FROM Birthdays WHERE MemberID=:memberID and GuildID=:guildID""",
                            {"memberID": ctx.author.id, "guildID": ctx.guild.id},
                        )
                        bday = datetime.datetime(
                            day=int(args[0]), month=int(args[1]), year=int(args[2])
                        )
                        await curr.execute(
                            """INSERT INTO Birthdays (MemberID, GuildID, Birthday) 
                        ...
                        await db.commit()
                        await ctx.send(
                            "Your birthday has been set to {}/{}/{}".format(
                        ...
                        return
                await ctx.send(
                    "Uhhh what the hell even happened. How do you even get here?"
                )
                pass
    ...
    async def sendBirthdayMessages(self):
        ...
        async with aiosqlite.connect(self.RT_DATABASE) as db:
            async with db.execute(
                """SELECT MemberID, GuildID FROM Birthdays WHERE Birthday LIKE :currentDate""",
                ...
                async for row in cursor:
                    ...
                    if server_info != None:
                        channel = discord.utils.get(
                            self.client.get_all_channels(), id=server_info[0]
                        )
                        await channel.send(
                            "Hey, happy birthday to <@{}>!".format(row[0])
                        )
