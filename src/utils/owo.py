...
class OwO(commands.Cog, name="Why? I don't even know why these exists!"):
    ...
    def __init__(self, client: discord.ext.commands.Bot):
        self.client = client
        db = sqlite3.connect(RT_DATABASE, detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES)
        db.execute('''CREATE TABLE IF NOT EXISTS Marriage ([ID] INTEGER, [GuildID] INTEGER, 
        ...
        async with ctx.channel.typing():
            ...
            if len(ctx.message.mentions) != 1:
                ...
                def check_second_side(msg):
                    return msg.author.id == ctx.message.mentions[0].id and msg.content == 'Yes!'

                async with aiosqlite.connect(RT_DATABASE, detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES) as db:
                    async with db.execute('''SELECT GuildID, FirstSide FROM Marriage WHERE `GuildID` = :guildID AND `SecondSide` = :author''', 
                    {'guildID': ctx.guild.id, 'author': ctx.message.author.id}) as rows:
                        this_row = await rows.fetchone()
                        if (this_row):
                            #print(this_row)
                            second_member = discord.utils.find(lambda m: m.id == this_row[1], ctx.guild.members)
                            await ctx.send(f"You already have your soul attached to {second_member}. What are you doing?")
                            return

                    async with db.execute('''SELECT GuildID, SecondSide FROM Marriage WHERE `GuildID` = :guildID AND `FirstSide` = :author''', 
                    {'guildID': ctx.guild.id, 'author': ctx.message.author.id}) as rows:
                        this_row = await rows.fetchone()
                        if (this_row):
                            #print(this_row)
                    ...
                    async with db.execute('''SELECT GuildID, FirstSide FROM Marriage WHERE GuildID = :guildID AND `SecondSide` = :target''', 
                    {'guildID': ctx.guild.id, 'target': ctx.message.mentions[0].id}) as rows:
                        this_row = await rows.fetchone()
                        if (this_row):
                            #print(this_row)
                            first_member = discord.utils.find(lambda m: m.id == this_row[1], ctx.guild.members)
                            await ctx.send(f"Sorry, but their soul has already been attached to {first_member}. I am magic, but I can't do anything for you")
                            return

                    async with db.execute('''SELECT GuildID, SecondSide FROM Marriage WHERE GuildID = :guildID AND `FirstSide` = :target''', 
                    {'guildID': ctx.guild.id, 'target': ctx.message.mentions[0].id}) as rows:
                        this_row = await rows.fetchone()
                        if (this_row):
                            #print(this_row)
                    ...
                    this_embed = OwO.construct_marriage_proposal(ctx.message.author, ctx.message.mentions[0])
                    await ctx.send(embed=this_embed)
                    await ctx.send(f"{ctx.message.mentions[0]} has 15 seconds to answer!") 
                    try:
                        await self.client.wait_for('message', check=check_second_side, timeout=15)
                    except asyncio.TimeoutError:
                        await ctx.send("Oh no! They didn't seem to care :(")
                        return
                    else:
                        await db.execute('''INSERT INTO Marriage (`GuildID`, `FirstSide`, `SecondSide`, `StartDate`) VALUES  
                        (:guildID, :first_member, :second_member, :today)''', 
                        {'guildID': ctx.guild.id, 'first_member': ctx.message.author.id, 'second_member': ctx.message.mentions[0].id, 'today': datetime.datetime.now()})
                        await db.commit()

                        marriage_photo = await OwO.generate_marry_image(ctx.message.author, ctx.message.mentions[0])
                        await ctx.send(f"Yay! Congratulations, {ctx.message.author.mention} ❤️ {ctx.message.mentions[0].mention}. We wish they have a sweet time together! 💍", file = marriage_photo)
    ...
    async def divorce(self, ctx):
        ...
        async with aiosqlite.connect(RT_DATABASE, detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES) as db:
            async with db.execute('''SELECT GuildID, FirstSide, SecondSide FROM Marriage WHERE `GuildID` = :guildID AND (`FirstSide` = :author OR `SecondSide` = :author)''', 
            {'guildID': ctx.guild.id, 'author': ctx.message.author.id}) as rows:
                this_row = await rows.fetchone()
                if (this_row):
                    first_member = discord.utils.find(lambda m: m.id == this_row[1], ctx.guild.members)
                    second_member = discord.utils.find(lambda m: m.id == this_row[2], ctx.guild.members)
                    await ctx.send(f"You are in a relationship. You know, the one between {first_member} and {second_member}...")
                    await ctx.send(f"If you are really sure about this, please say 'DO IT' ")

                    try:
                        await self.client.wait_for('message', check=check_divorce, timeout=15)
                    except asyncio.TimeoutError:
                        await ctx.send("You did not reply to the request")
                        pass
                    else:
                        await db.execute('''DELETE FROM Marriage WHERE GuildID=:guildID AND (`FirstSide`=:author OR `SecondSide`=:author)''',
    ...
    async def on_member_remove(self, member):
        async with aiosqlite.connect(RT_DATABASE, detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES) as db:
            async with db.cursor() as curr:
                await curr.execute('''DELETE FROM Marriage WHERE GuildID=:guildID AND (FirstSide=:memberID OR SecondSide=:memberID)''',
    ...
    async def on_guild_remove(self, guild: discord.Guild):
        async with aiosqlite.connect(RT_DATABASE, detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES) as db:
            async with db.cursor() as curr:
                await curr.execute('''DELETE FROM Marriage WHERE GuildID=:guildID''',
                {'guildID': guild.id})
                await db.commit()
    ...
    async def generate_marry_image(first_member: discord.Member, second_member: discord.Member) -> discord.File:
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=15)) as session:
            ...
            draw.ellipse((offset_2[0] - 2, offset_2[1] - 2, 
                        (offset_2[0])+icon_w_2 + 2, (offset_2[1])+icon_h_2 + 2), 
                        fill='#36393f', outline="#ffffff", width=2)
            blurple.paste(icon_2, offset_2, icon_2)
    ...
    def construct_marriage_proposal(first_member: discord.Member, second_member: discord.Member) -> discord.Embed:
        ...
        embed.add_field(
            name = "Member",
            value = f"```{first_member}```",
            inline = False
        )
        embed.add_field(
            name = "Has proposed",
            value = f"```{second_member}```",
            inline = False
        )
        embed.add_field(
