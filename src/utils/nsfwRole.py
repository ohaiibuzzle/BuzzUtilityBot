...
class NSFWRoleManagement(commands.Cog):
    ...
    def __init__(self, client):
        ...
        db.execute(
            """CREATE TABLE IF NOT EXISTS `NSFWBans` ([GuildID] INTEGER, [MemberID] INTEGER, [BanReason] TIMESTAMP, PRIMARY KEY ("GuildID", "MemberID"))"""
        )
        db.commit()
    ...
    async def setNSFWRole(self, ctx: commands.Context):
        ...
        if ctx.message.role_mentions.__len__() == 1:
            ...
            await ctx.send(
                f"I am going to set the {role.mention} as the server's NSFW role. Confirm? [Yes/No]"
            )
            try:
                ...
                async with aiosqlite.connect(self.RT_DATABASE) as db:
                    await db.execute(
                        """DELETE FROM NSFWRoles WHERE GuildID = :guildID""",
                        {"guildID": ctx.guild.id},
                    )
                    await db.execute(
                        """INSERT INTO NSFWRoles (GuildID, RoleID) VALUES (:guildID, :roleID)""",
    ...
    async def delNSFWRole(self, ctx: commands.Context):
        ...
        await ctx.send(
            f"I am going to remove the server's NSFW role. Confirm? [Yes/No]"
        )
        try:
            await self.client.wait_for("message", check=verify_nsfw_set, timeout=15)
        except asyncio.TimeoutError:
            ...
            async with aiosqlite.connect(self.RT_DATABASE) as db:
                ...
                await db.execute(
                    """DELETE FROM NSFWBans WHERE GuildID = :guildID""",
                    {"guildID": ctx.guild.id},
                )
                await db.commit()
                await ctx.send("Done")
    ...
    async def requestNSFW(self, ctx):
        ...
        async with aiosqlite.connect(self.RT_DATABASE) as db:
            async with db.execute(
                """SELECT * FROM NSFWBans WHERE GuildID=:guildID AND MemberID=:memberID""",
                {"guildID": ctx.guild.id, "memberID": ctx.author.id},
            ) as query:
                rows = await query.fetchall()
                if rows.__len__() != 0:
                    return await ctx.send(
                        f"You have been banned from getting NSFW on this server due to: {rows[0][2]}"
                    )

            async with db.execute(
                """SELECT GuildID, RoleID from NSFWRoles WHERE GuildID = :guildID""",
                {"guildID": ctx.guild.id},
            ) as cursor:
                ...
                if rows.__len__() == 0:
                    ...
                    await ctx.author.send(
                        f"You are requesting NSFW access to {ctx.guild.name}"
                    )
                    await ctx.author.send(
                        f"By replying `I agree` to this message, you agree that:\n"
                    ...
                    await ctx.author.send(
                        "You have 20s to reply to this request or it will be aborted automatically"
                    )
                try:
                    await self.client.wait_for(
                        "message", check=validate_opt_in, timeout=20
                    )
                except asyncio.TimeoutError:
    ...
    async def nsfwban(self, ctx: commands.Context, *, reason: str):
        ...
        async with aiosqlite.connect(self.RT_DATABASE) as db:
            await db.execute(
                """INSERT INTO NSFWBans (GuildID, MemberID, BanReason) VALUES(:guildID, :memberID, :banReason)""",
                {
                    "guildID": ctx.guild.id,
    ...
    async def nsfwunban(self, ctx: commands.Context):
        ...
        async with aiosqlite.connect(self.RT_DATABASE) as db:
            await db.execute(
                """DELETE FROM NSFWBans WHERE GuildID = :guildID AND MemberID = :memberID""",
